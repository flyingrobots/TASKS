...
func Build(tasks []m.Task, edges []m.Edge, minConfidence float64) (m.DagFile, error) {
    ...
    for _, e := range edges {
        ...
        if _, ok := idx[e.From]; !ok { continue }
        if _, ok := idx[e.To]; !ok { continue }
    ...
    dfs = func(u int) bool {
        seen[u] = 1
        for _, v := range adj[u] {
            if seen[v] == 1 { return true } // cycle
            if seen[v] == 0 && dfs(v) { return true }
        }
        seen[u] = 2
        return false
    }
    ...
    indeg2 := make([]int, n)
    copy(indeg2, indeg)
    for i := 0; i < n; i++ { if indeg2[i] == 0 { q = append(q, i) } }
    topo := []int{}
    depth := make([]int, n)
    for len(q) > 0 {
        u := q[0]; q = q[1:]
    ...
    type K struct{ F, T string; Ty string }
    kept2 := []K{}
    for key := range keepEdge {
        u, v := key[0], key[1]
        kept2 = append(kept2, K{F: tasks[u].ID, T: tasks[v].ID, Ty: lookupEdgeType(kept, tasks[u].ID, tasks[v].ID)})
    }
    sort.Slice(kept2, func(i, j int) bool { if kept2[i].F == kept2[j].F { return kept2[i].T < kept2[j].T }; return kept2[i].F < kept2[j].F })
    for _, e := range kept2 {
    ...
    for i := 0; i < n; i++ {
        d := depth[i]
        depthCount[d]++
        if depthCount[d] > maxW { maxW = depthCount[d] }
    }
...
func lookupEdgeType(es []edgeRec, f, t string) string { // edgeRec is defined above
    for _, e := range es { if e.From == f && e.To == t { return e.Type } }
    return ""
}
